class Scatterplot {
    ...
    renderVis() {
      ...
      const symbols = vis.chart.selectAll('.symbol')
        ...
        .join('path')
          .attr('class', 'symbol')
          .attr('transform', d => `translate(${vis.xScale(jitter(vis.xValue(d)))}, ${vis.yScale(vis.yValue(d))})`) // add jitter function around the x coordinate
    ...
    function jitter(value) {
        ...
        var num = Math.floor(Math.random()*1.5) + 1; // i decreased the factor here so that the points don't run outside of the scatterplot 
